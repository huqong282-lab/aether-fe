...
Total 31 Task Frontend tercakup pada dokumen ini, mencakup seluruh layar dan komponen yang telah dirinci pada UI/UX Specification (Fase 7), termasuk dukungan poll multiple-choice (FE16.1), integrasi keamanan sisi client (FE17.1), **EPIC 8 — Direct Message** (FE20.1-FE20.3), serta perluasannya **Friend & Social UI** (FE21.1-FE21.4) dan **DM Calls & Group Management UI** (FE22.1-FE22.2) — lihat Addendum-Direct-Message.docx (v2) untuk detail spesifikasi lengkap.
...
**Subtask:**
- [x] Inisialisasi proyek Vite (React + TypeScript template).
- [x] Install & konfigurasi TailwindCSS beserta custom theme (warna, font) dari UI/UX Specification.
- [x] Setup React Router dengan route dasar & layout shell.
- [x] Integrasikan Biome untuk linting frontend.
**Checklist Verifikasi:**
- [x] npm run dev berjalan tanpa error
- [x] Warna & font sesuai token UI/UX Specification
- [x] Navigasi antar route dasar berfungsi
...
**Subtask:**
- [x] Setup QueryClient & Provider TanStack Query.
- [x] Buat Zustand store authStore (accessToken in-memory, user profile).
- [x] Buat HTTP client wrapper dengan interceptor refresh token.
- [x] Buat Zustand store uiStore (modal, toast, sidebar toggle state).
**Checklist Verifikasi:**
- [x] Access token otomatis di-refresh saat kedaluwarsa
- [x] Request gagal 401 setelah refresh gagal mengarahkan ke /login
...
**Subtask:**
- [x] Bangun komponen form Login & Register (toggle dalam satu card).
- [x] Integrasikan validasi Zod di sisi client (mirror validasi backend).
- [x] Hubungkan ke endpoint POST /auth/register & /auth/login via TanStack Query mutation.
- [x] Tangani & tampilkan error dari API (mis. 409 Conflict, 400 Validation).
**Checklist Verifikasi:**
- [x] Register & login berhasil end-to-end dari UI
- [x] Pesan error ditampilkan sesuai format error API
...
**Subtask:**
- [x] Bangun komponen daftar sesi dengan TanStack Query (GET /auth/sessions).
- [x] Implementasikan aksi revoke sesi (DELETE /auth/sessions/{id}) dengan optimistic update.
- [x] Tambahkan modal konfirmasi sebelum revoke.
**Checklist Verifikasi:**
- [x] Revoke sesi berhasil & list ter-update otomatis
- [x] Sesi yang sedang aktif (current session) ditandai berbeda
...
**Subtask:**
- [x] Bangun komponen Server Rail dengan state server aktif (Zustand).
- [x] Bangun modal Create Server (form nama & ikon) terhubung ke POST /servers.
- [x] Bangun modal Join Server (jika menggunakan invite link/code).
**Checklist Verifikasi:**
- [x] Server baru muncul di Server Rail tanpa reload
- [x] Server aktif ter-highlight sesuai desain UI/UX Specification
...
**Subtask:**
- [x] Bangun komponen tree Category → Channel dengan ikon per tipe (#, 🔊, 📢, 🧵).
- [ ] Bangun modal create/edit category & channel.
- [x] Implementasikan state unread per channel (Zustand/TanStack Query cache).
**Checklist Verifikasi:**
- [x] Channel sidebar menampilkan struktur sesuai data API
...
**Subtask:**
- [x] Buat hook useWebSocket dengan state connecting/connected/disconnected.
- [x] Implementasikan strategi reconnect exponential backoff.
- [x] Implementasikan sinkronisasi ulang state (presence, pesan) setelah reconnect.
- [x] Buat event dispatcher untuk meneruskan event ke Zustand store/TanStack Query cache terkait.
**Checklist Verifikasi:**
- [x] Koneksi pulih otomatis setelah jaringan sempat terputus
- [x] Tidak ada pesan yang hilang secara permanen setelah reconnect (diuji manual)
...
## Feature S20 — Direct Message UI
**FE20.1 — DM List & New DM Modal**
| **Deskripsi** | **Halaman/panel daftar percakapan Direct Message (1:1 & group) yang diakses melalui item "DM" pada navigasi utama, beserta modal untuk memulai percakapan baru dengan user lain.** |
| --- | --- |
| **Acceptance Criteria** | **1. Daftar DM terurut berdasarkan aktivitas terbaru, dengan preview pesan terakhir.  2. Memulai DM ke user yang sudah punya percakapan membuka percakapan yang sama, bukan membuat entri baru.** |
| **Dependency** | **FE1.2, FE9.1** |
| **Priority** | **Must (1:1 DM), Should (Group DM)** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **7 jam** |
| **Definition of Done** | **Pengguna dapat melihat daftar DM dan memulai percakapan baru dari UI.** |
**Subtask:**
- [ ] Bangun komponen daftar DM (GET /dm) dengan preview pesan terakhir & unread badge.
- [ ] Bangun modal "New DM" dengan pencarian user.
- [ ] Integrasikan ke POST /dm untuk memulai/mengambil percakapan.
**Checklist Verifikasi:**
- [ ] Daftar DM ter-update realtime saat ada pesan baru masuk
- [ ] Memulai DM ke user yang sama tidak membuat entri duplikat di daftar

**FE20.2 — DM Conversation View**
| **Deskripsi** | **Tampilan percakapan DM yang memakai ulang komponen Message List & Composer dari Chat UI Core (FE6.1), disesuaikan untuk konteks tanpa Server/Channel Sidebar.** |
| --- | --- |
| **Acceptance Criteria** | **1. Kirim/edit/hapus/reaksi pesan pada DM menggunakan komponen yang sama dengan channel biasa, tanpa duplikasi kode signifikan.  2. Header percakapan menampilkan nama & presence lawan bicara (1:1) atau daftar partisipan (group).** |
| **Dependency** | **FE20.1, FE6.1, FE8.1** |
| **Priority** | **Must** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **Percakapan DM dapat digunakan sepenuhnya dari UI selayaknya text channel biasa.** |
**Subtask:**
- [ ] Adaptasi komponen Message List & Composer agar menerima channel bertipe dm/group_dm.
- [ ] Bangun header percakapan DM (nama/presence lawan bicara atau daftar partisipan group).
- [ ] Uji reuse komponen reaction picker & attachment pada konteks DM.
**Checklist Verifikasi:**
- [ ] Kirim/terima pesan DM realtime berfungsi sama seperti channel biasa
- [ ] Presence lawan bicara tampil akurat pada header percakapan 1:1

**FE20.3 — Integrasi Notifikasi DM**
| **Deskripsi** | **Memastikan pesan DM baru memicu badge unread & toast notifikasi yang sama seperti notifikasi mention pada channel biasa (FE9.1).** |
| --- | --- |
| **Acceptance Criteria** | **1. Item navigasi "DM" menampilkan badge jumlah percakapan dengan pesan belum dibaca.  2. Toast notifikasi muncul untuk pesan DM baru saat pengguna sedang aktif di bagian lain aplikasi.** |
| **Dependency** | **FE20.2, FE9.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Easy** |
| **Estimasi Waktu** | **4 jam** |
| **Definition of Done** | **Notifikasi DM konsisten dengan pola notifikasi yang sudah ada pada channel biasa.** |
**Subtask:**
- [ ] Perluas store notifikasi (FE9.1) agar menghitung unread per percakapan DM.
- [ ] Tambahkan badge pada item navigasi DM (Server Rail/Bottom Navigation).
**Checklist Verifikasi:**
- [ ] Badge unread DM akurat & ter-update realtime
- [ ] Toast notifikasi DM tampil konsisten dengan notifikasi channel biasa

## Feature S21 — Friend & Social UI
**FE21.1 — Friends Page & Add Friend Modal**
| **Deskripsi** | **Halaman Friends dengan tab Online/All/Pending/Blocked, dan modal Add Friend dengan pencarian username/email.** |
| --- | --- |
| **Acceptance Criteria** | **1. Tab Online hanya menampilkan teman dengan presence aktif, terintegrasi dengan store presence (FE8.1).  2. Mengirim friend request menampilkan feedback status langsung (terkirim/sudah berteman/sudah pernah dikirim).** |
| **Dependency** | **FE1.2, FE8.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **8 jam** |
| **Definition of Done** | **Pengguna dapat mengelola pertemanan sepenuhnya dari UI sesuai FR-DM-04/05.** |
**Subtask:**
- [ ] Bangun komponen tab Online/All/Pending/Blocked (GET /friends?tab=).
- [ ] Bangun modal Add Friend dengan pencarian & submit ke POST /friends/requests.
- [ ] Bangun aksi accept/decline/cancel/remove pada tiap tab yang relevan.
**Checklist Verifikasi:**
- [ ] Tab Pending memisahkan permintaan masuk & keluar dengan aksi berbeda
- [ ] Status online pada tab Online ter-update realtime

**FE21.2 — Block & Privacy Settings UI**
| **Deskripsi** | **UI blokir pengguna (dari popup profil/tab Blocked) dan sub-halaman Privacy Settings untuk mengatur dmPrivacy.** |
| --- | --- |
| **Acceptance Criteria** | **1. Blokir dari popup profil langsung memindahkan user ke tab Blocked.  2. Toggle privacy Everyone/Friends Only tersimpan & tercermin pada percobaan DM berikutnya.** |
| **Dependency** | **FE21.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Easy** |
| **Estimasi Waktu** | **5 jam** |
| **Definition of Done** | **Block & privacy setting dapat dikelola penuh dari UI sesuai FR-DM-06/07.** |
**Subtask:**
- [ ] Bangun aksi block/unblock terintegrasi ke POST/DELETE /users/{userId}/block.
- [ ] Bangun sub-halaman Privacy Settings pada User Settings (PATCH /users/me/privacy).
**Checklist Verifikasi:**
- [ ] User yang diblokir tidak muncul lagi sebagai opsi New DM
- [ ] Perubahan privacy tersimpan & bertahan setelah reload

**FE21.3 — Message Requests Inbox**
| **Deskripsi** | **Daftar terpisah untuk DM berstatus pending_request, dengan badge tersendiri dan aksi cepat Accept/Decline.** |
| --- | --- |
| **Acceptance Criteria** | **1. Message Request tidak tercampur dengan DM List utama (FE20.1).  2. Accept memindahkan percakapan ke DM List utama tanpa reload halaman.** |
| **Dependency** | **FE20.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **Message Requests berfungsi penuh dari UI sesuai FR-DM-08.** |
**Subtask:**
- [ ] Bangun komponen daftar Message Requests (GET /dm/requests) dengan badge terpisah dari DM List.
- [ ] Implementasikan aksi Accept (POST /dm/{channelId}/accept) & Decline (DELETE /dm/{channelId}/decline).
**Checklist Verifikasi:**
- [ ] Badge Message Requests akurat & terpisah dari badge DM biasa
- [ ] Decline menghapus percakapan dari daftar tanpa muncul kembali

**FE21.4 — User Profile Popup**
| **Deskripsi** | **Kartu popup profil pengguna saat avatar/nama diklik di mana pun dalam aplikasi, dengan tombol aksi kontekstual sesuai status relasi.** |
| --- | --- |
| **Acceptance Criteria** | **1. Tombol aksi berubah sesuai status relasi (Send Message / Add Friend / Remove Friend / Block / Unblock).  2. Menampilkan daftar server bersama & teman bersama (bila ada).** |
| **Dependency** | **FE21.1** |
| **Priority** | **Could** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **Popup profil berfungsi penuh dari UI sesuai FR-DM-11.** |
**Subtask:**
- [ ] Bangun komponen popup profil (GET /users/{userId}/profile).
- [ ] Implementasikan tombol aksi kontekstual berdasarkan relationship status dari response API.
- [ ] Trigger popup dari seluruh titik avatar (Message List, Member List, Friends Page).
**Checklist Verifikasi:**
- [ ] Popup dapat dipicu konsisten dari seluruh titik avatar di aplikasi
- [ ] Tombol aksi sesuai status relasi terkini (tidak menampilkan "Add Friend" pada user yang sudah berteman)

## Feature S22 — DM Calls & Group Management UI
**FE22.1 — Call UI dalam DM**
| **Deskripsi** | **Tombol panggilan suara/video pada header DM Conversation View, memakai ulang komponen voice/video control bar & grid peserta dari Voice/Video Channel (FE13.1/FE14.1).** |
| --- | --- |
| **Acceptance Criteria** | **1. Tombol call hanya aktif bila seluruh participant berstatus 'accepted'.  2. UI panggilan identik secara visual dengan panggilan voice/video channel server.** |
| **Dependency** | **FE13.1, FE14.1, FE20.2** |
| **Priority** | **Could** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **Panggilan dalam DM berfungsi dari UI sesuai FR-DM-09.** |
**Subtask:**
- [ ] Tambahkan tombol call pada header DM Conversation View.
- [ ] Integrasikan ke POST /dm/{channelId}/call/token & reuse komponen voice/video control bar.
**Checklist Verifikasi:**
- [ ] Panggilan DM 1:1 & group berfungsi end-to-end dari UI
- [ ] Komponen call di DM konsisten secara visual dengan call di voice channel server

**FE22.2 — Group DM Management UI**
| **Deskripsi** | **Modal pengaturan Group DM: ubah nama/ikon, daftar peserta dengan aksi remove, dan tombol Leave Group.** |
| --- | --- |
| **Acceptance Criteria** | **1. Modal ini hanya muncul untuk percakapan bertipe group_dm.  2. Leave Group mengembalikan pengguna ke DM List tanpa error meski grup masih memiliki peserta lain.** |
| **Dependency** | **FE20.2** |
| **Priority** | **Could** |
| **Estimasi Kesulitan** | **Easy** |
| **Estimasi Waktu** | **5 jam** |
| **Definition of Done** | **Group DM dapat dikelola penuh dari UI sesuai FR-DM-10.** |
**Subtask:**
- [ ] Bangun modal Group DM Settings (rename/icon via PATCH /dm/{channelId}).
- [ ] Bangun UI daftar peserta dengan aksi add/remove (POST/DELETE /dm/{channelId}/participants).
- [ ] Bangun tombol Leave Group dengan konfirmasi.
**Checklist Verifikasi:**
- [ ] Rename/ubah ikon grup tersinkron realtime ke seluruh peserta
- [ ] Leave Group berhasil tanpa mengganggu peserta lain yang masih ada di grup
...
- Direct Message (EPIC 8) memakai ulang komponen Message List, Composer, reaction picker, dan attachment dari Chat UI Core (FE6.1) alih-alih membangun UI percakapan terpisah, agar konsisten secara visual dan mengurangi duplikasi kode.
- Fitur sosial nice-to-have (Friend, Block, Privacy, Message Requests, Call dalam DM, Group DM Management) ditambahkan sebagai Feature S21-S22 dengan prioritas Should/Could, memakai ulang komponen Voice/Video Channel (FE13.1/FE14.1) untuk Call dalam DM.
...
- Apakah Group DM (FE20.1) benar-benar dibutuhkan pada rilis ini atau dapat ditunda, selaras dengan pertanyaan yang sama pada Task Checklist Backend (T20.1).
...
- FE20.2 mengasumsikan komponen Message List & Composer (FE6.1) cukup generik untuk dipakai ulang pada konteks DM; bila ternyata banyak asumsi channel-only tertanam di komponen tersebut, effort FE20.2 berisiko lebih besar dari estimasi.
...
- UI/UX Specification (Fase 7) belum diperbarui secara penuh dengan wireframe/spesifikasi layar DM secara rinci — dirangkum pada dokumen Addendum terpisah, bukan regenerasi penuh dokumen tersebut.

# Pertanyaan untuk Stakeholder

- Apakah Task Checklist Frontend ini, termasuk EPIC 8 — Direct Message, sudah dianggap lengkap dan selaras dengan Task Checklist Backend, sehingga kedua checklist siap dijadikan acuan implementasi bersama-sama per sprint?
